...
| **What was changed manually** | I reviewed code to understand and then direct revision of its structure, though I often did this collaboratively with Claude Code via natural language. I did sometimes revise code manually, often to enforce formatting preferences or make minor adjustments or edit comments and doc strings.I revised Tier 3 evaluation criterion (changed from confidence < 0.70 to no overconfident accept/reject), implemented secrets management, implemented an encoding workaround after havingissues with one of the input trees. |
| **What was verified independently** | Parser output verified against GEDCOM files opened in text editor. JFK family relationships verified against public biographical sources. Moore family tree relationships verified against my personal family knowledge. Queen Victoria parentage verified against historical record. Habsburg Philip II parentage verified against historical record. Geographic distances spot-checked against online calculators. Tier 1 trap cases verified by manual date arithmetic. |

---

### Entry 8 — Pipeline Testing on My Family Tree
...
| **Prompt summary** | I described (over multiple turns, adding in features along the way) a layout including four-tab scaffolding, file-picker dropdowns scanning `data/`, the Audit tab with two-pass deterministic plus LLM workflow, DNA file dropdown, data-availability banner, and an explicit mode selector. The build proceeded incrementally with stop-for-review checkpoints between each tab. |
| **What was changed manually** | I made layout adjustments, copy edits, chose the colour palette for the verdict nodes, placement of progress indicators, the wording of the data-availability banner, and did some CSS tuning. |
...
### Entry 12 — DNA Analyst Agent (Reporter Mode)
...
| **Prompt summary** | I described the purpose and needed features in a request to build a DNA Analyst node that parses the two CSV formats, looks up each match's name in the GEDCOM, and produces a summary section. |
| **What was changed manually** | I determined and implemented column-mapping logic between GEDmatch and MyHeritage formats, fuzzy-match thresholds for cross-referencing match names against GEDCOM persons, decisions about which match-list fields to surface in the report. |
...
| **Prompt summary** | After reviewing the reporter-only output, I felt the DNA path was greatly under-used. DNA evidence appeared in the report but never influenced any verdict. It seemed worth restructuring and expanding the scope for the desired aspirational feature. I broke down what was desirable about the Reporter mode and where it did not meet desired needs, identified inputs and outputs to drive discussion of options and feasibility. After assessing options, described desired feasible solution logic and scope, and list of avoidances.|
| **What was changed manually** | I edited how DNA findings are phrased in the evidence chain, the cM-consistency verdict thresholds (consistent / inconsistent / inconclusive), governed whether DNA evidence should weaken or support a hypothesis by default. |
...
|-------|---------|
| **Tool** | Claude Code (Anthropic) |
| **Purpose** | Extending the Record Scout beyond GEDCOM-only retrieval. Added FindAGrave (HTML scrape via BeautifulSoup), Wikidata (SPARQL endpoint), and WikiTree (public REST API). Each retrieved record is tagged with `source_type` so downstream agents, the Critic in particular, can weight independent corroboration appropriately rather than treating one user-uploaded GEDCOM as the only source of truth. |
| **Prompt summary** | Identified potential sources one by one and asked for integration strategies. Then narrowed list and sought follow up development of integration solution within Scout agent.FamilySearch was scoped originally but deprioritized after API-key registration friction; Sought equivalent independent coverage came from Wikidata and WikiTree. |
| **What was changed manually** | I edited search-result filtering thresholds, rate-limit handling, defensive handling of HTML structure changes on FindAGrave. |
...
| **Prompt summary** | I described the goal of surfacing gaps in evidence in a tree as a list of leads or focus points for a user. Solutions were discussed, and then selected the design elements I wanted.  |
| **What was changed manually** | I tuned the data-richness threshold (`min_data_fields`) for which gaps are worth investigating, the scoring weights for parent candidates, the auto-generated query template for each gap. |
...
| **Prompt summary** | Described need for root-based scan of chain of evidence backward to a variable degree. |
| **What was changed manually** | I chose the Pass-1 severity thresholds (impossible / flagged / ok), shaped the heuristic for "questionable" that determines which relationships go to Pass 2, and tuned the report format for downloaded audit reports. |
...
| **Prompt summary** | Phase 2 feedback noted that the Critic's accept/reject was binary and obscured low-confidence acceptances. Asked Claude Code to formalize the escalation conditions and integrate them into the Final Report Writer. |
| **What was changed manually** | Trigger 3 was narrowed during review to fire only when the same subject has both `accept` and `reject` since a mix of `accept` and `flag_uncertain` is normal. (The Critic is more confident about some relationships than others.) and was producing false escalations in early traces.|
...
### Entry 18 — Critic Isolation Enforcement
...
| **Prompt summary** | Directed goal of preventing leakage and formalizing separation of Hypothesizer reasoning. |
| **What was changed manually** | I defined the exact partition of fields between PUBLIC and INTERNAL (in particular whether the Hypothesizer's stated weaknesses count as PUBLIC, determined they should since they are part of the Hypothesizer's openly declared limitations and a useful signal to the Critic). |
...
### Entry 19 — Critic Model Upgrade: Sonnet 4.6 → Opus 4.6 → Opus 4.7

| Field | Details |
|-------|---------|
| **Tool** | Claude Code (Anthropic) — model swap in `agents/adversarial_critic.py` |
| **Purpose** | Tuning Critic model selection independently from the rest of the pipeline. Initial production used Claude Sonnet 4.6 across all five agents; comparison runs against Claude Opus 4.6 produced more conservative confidence ratings; the final production setting is Claude Opus 4.7 for the Critic only, keeping Sonnet 4.6 for Record Scout, Profile Synthesizer, Hypothesizer, and DNA Analyst. |
| **Prompt summary** | Requested model swap for specific pipeline elements. |
| **What was changed manually** | The final decision to keep the Critic on Opus 4.7 and the others on Sonnet 4.6 was a cost/quality tradeoff judgment.|
...
| **Prompt summary** | Requested experiment and results comparing pipeline using Anthropic-only models vs. versions with OpenAI models replacing either the Sonnet-based agents or the Opus-based Critic. |
| **What was changed manually** | I specified the prompt template adjustment for the OpenAI variant, updated for parsing differences in JSON-mode outputs, governed scoring of disagreements. |
...
### Entry 21 — Multi-Critic Ensemble
...
| **Prompt summary** | I provided experiment design structure and goals and desired models. I defined output metric.|
| **What was changed manually** | aggregation rule, prompt parity tuning across the three vendors |
...
| **Prompt summary** | Each fix was a follow-up after observing the failure. I described why the current behavior was a failure and what was required for success. |
| **What was changed manually** | I identified all three bugs in trace inspection or output review, flagged them, and reviewed each fix before merging. |
...
| **Prompt summary** | I described absolute restrictions and sharing goals and asked for security solutions and feasibility assessments to determine what scope could accommodate. Once assessed, I requested design elements and then directed audits. |
| **What was changed manually** | Curated the synthetic match lists to map onto the public GEDCOM persons, using realistic shared-cM values consistent with the Shared cM Project distribution. |
...
| **Prompt summary** | Requested preservation of most of diagram structure while implementing revision reflecting design changes. |
| **What was changed manually** | Manually edited the Mermaid for visual layout (node positions, edge routing), color scheme. |
...
|------|---------|-------------|-----------------|
| Claude | Opus 4.6 & 4.7 (Anthropic) | 1, 2, 3 | Project design, code generation, document drafting, debugging, evaluation design |

---

## Disclosure Statement

Generative AI (Claude Opus 4.6 & 4.7) were used extensively as a collaborative tool throughout this project. I maintained responsibility for all design decisions, domain judgment, evaluation criteria, and final content. AI-generated outputs were reviewed, revised where necessary, and independently verified before inclusion. My domain expertise in genealogy (active hobbyist) and genetics (M.S. in Cellular and Molecular Biology) informed the evaluation of AI-generated content throughout.
